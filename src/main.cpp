...
//  PathPlanningStrategy *pathPlanningStrategy = new PathPlanningFollowLineStrategy(map_waypoints_x, map_waypoints_y, map_waypoints_s, map_waypoints_dx, map_waypoints_dy, Lane(1));
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy, &pathPlanningStrategy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			DriveEnvironment drivingStatus = DriveEnvironment(
                car_x, car_y, car_s, car_d, car_yaw, car_speed, end_path_s, end_path_d,
            ...
            pathPlanningStrategy->planningPath(drivingStatus, next_x_vals, next_y_vals);
